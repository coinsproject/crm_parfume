...
- Все функции обновления теперь через единый скрипт full_update.sh""",
    ...
    {
        "version": "1.3.4",
        "title": "Версия 1.3.4 - Исправление поиска и расчета цен",
        "description": "Исправлены критические ошибки в поиске товаров и расчете цен в модальном окне добавления товаров в заказ",
        "release_type": "patch",
        "release_date": date(2026, 1, 27),
        "changes": """Исправлено:
- Поиск товаров с кириллицей в модальном окне добавления товаров в заказ
  * Добавлена поддержка FTS5 для корректного поиска с кириллицей
  * Улучшен fallback-поиск с разбиением запроса на токены
  * Теперь поиск работает для запросов типа "Byredo Vetyver Hand Cream Женский крем для рук (тестер) 100 мл"

- Расчет цены в модальном окне поиска товаров
  * Исправлена логика получения базовой цены (используется та же логика, что и при добавлении товара)
  * Цена теперь рассчитывается с учетом всех накруток (партнера и клиента)
  * Цена в модальном окне теперь совпадает с ценой при добавлении товара в заказ
  * Исправлена проблема, когда показывалась цена 4106,40 вместо правильной 4229,59

Технические детали:
- Использование FTS5 для поиска с кириллицей в SQLite
- Унификация логики расчета цен между order_price_search_api и order_price_product_api
- Правильное применение накруток через get_total_markup_percent и calc_client_price""",
    },
]
